...
def _load_data(basedir, factor=None, width=None, height=None, load_imgs=True):
    ...
    poses[:2, 4, :] = np.array(sh[:2]).reshape([2, 1]) # 调整宽和高
...
def load_llff_data(basedir, factor=8, recenter=True, bd_factor=.75, spherify=False, path_zflat=False):
    ...
    poses = np.concatenate([poses[:, 1:2, :], -poses[:, 0:1, :], poses[:, 2:, :]], 1) # (3, 5, N), 对poses的第一次修改, [down, right, back] -> [right, up, back]
    ...
    poses[:,:3,3] *= sc # 对poses的第二次修改，调整尺度
    bds *= sc
    
    if recenter:
        poses = recenter_poses(poses) # 对poses的第三次修改, simply applies the inverse of this average pose to the dataset
                                      # (a rigid rotation/translation) so that the identity extrinsic matrix is looking
                                      # at the scene
    ...
    if spherify:
        ...
        c2w = poses_avg(poses) # computes a "central" pose for the dataset, based on using the mean translation,
                               # the mean z axis, and adopting the mean y axis as an "up" direction
                               # (so that Up x Z = X and then Z x X = Y)
...
#             zloc = np.percentile(tt, 10, 0)[2]
    ...
    print('HOLDOUT view is', i_test) # 拿走距离相机群中心最近的图像作测试
...
def load_colmap_depth(basedir, factor=8, bd_factor=.75):
    ...
    Errs = np.array([point3D.error for point3D in points.values()]) # 所有3D点的重投影误差
    Err_mean = np.mean(Errs)
    print('Mean Projection Error: ', Err_mean)

    poses = get_poses(images)
    _, bds_raw, _ = _load_data(basedir, factor=factor) # factor=8 downsamples original imgs by 8x, (2, N)
    bds_raw = np.moveaxis(bds_raw, -1, 0).astype(np.float32) # (N, 2)
    # print(bds_raw.shape)
    # Rescale if bd_factor is provided
    ...
    for id_im in range(1, len(images) + 1):
        ...
        coord_list = []
        weight_list = []
        for i in range(len(images[id_im].xys)): # 遍历img_{id_im}的2D点
            point2D = images[id_im].xys[i]
            id_3D = images[id_im].point3D_ids[i]
            if id_3D == -1: # 如果该2D点没有对应的3D点，跳过
                continue

            point3D = points[id_3D].xyz
            depth = (poses[id_im - 1, :3, 2].T @ (point3D - poses[id_im - 1, :3, 3])) * sc # 得到该点在img_{id_im}坐标系下的depth
            if depth < bds_raw[id_im-1,0] * sc or depth > bds_raw[id_im-1,1] * sc: # 如果该点的depth在bounds外，跳过
                continue

            err = points[id_3D].error
            weight = 2 * np.exp(-(err / Err_mean) ** 2)
            depth_list.append(depth)
            coord_list.append(point2D / factor) # 缩放2D坐标
            weight_list.append(weight) # 保存根据重投影误差得到的confidence
